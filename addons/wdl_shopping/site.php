...
//			pdo_delete('shopping_goods', array('id' => $id));
    ...
    public function doMobileConfirm() {
        ...
        if (checksubmit('submit')) {
            ...
            if (empty($address)) {
                $returl = $this->createMobileUrl('confirm', array('id' => $id, 'total' => $total));
                message('抱歉，请您填写收货地址！', $this->createMobileUrl('address', array('returnurl' => $returl, 'from' => 'confirm')));
            }
            // 商品价格
            $goodsprice = 0;
    ...
    public function doMobilePay() {
        ...
        if (checksubmit('codsubmit')) {
            ...
            if (!empty($this->module['config']['noticeemail'])) {
                ...
                $paytype = $order['paytype'] == '3' ? '线下支付' : '已付款';
            ...
            message('订单提交成功，请您收到线下支付！', $this->createMobileUrl('myorder'), 'success');
        }
        if (checksubmit()) {
    ...
    public function doMobileAddress() {
        ...
        if ($operation == 'post') {
            ...
            $ret = pdo_update('mc_members', array('mobile' => $data['mobile']), array('uid' => $data['uid']));
            if (!empty($id)) {
                unset($data['uniacid']);
                unset($data['uid']);
    ...
    private function ceshckContainer() {
        global $_W;
